...
namespace CosmosX.Core
{
    public class ReactorManager : IManager
    {
        ...
        public string ModuleCommand(IList<string> arguments)
        {
            ...
            switch (moduleType)
            {
                case "CryogenRod":
                    ...
                    break;
                case "HeatProcessor":
                    IAbsorbingModule heatProcessor = new HeatProcessor(currentId, additionalParameter);
                    ...
                    break;
                case "CooldownSystem":
                    IAbsorbingModule cooldownSystem = new CooldownSystem(currentId, additionalParameter);
        ...
        public string ReportCommand(IList<string> arguments)
        {
            ...
            {
                report = reactors[id].ToString();
            }
            if (modules.ContainsKey(id))
            {
                report = modules[id].ToString();
            }
        ...
        {
            long cryoReactorCount = this.reactors.Values.Count(r => r.GetType().Name == nameof(CryoReactor));

            long heatReactorCount = this.reactors.Values.Count(r => r.GetType().Name == nameof(HeatReactor));

            long energyModulesCount = this.modules.Values.Count(m => m is IEnergyModule);

            long absorbingModulesCount = this.modules.Values.Count(m => m is IAbsorbingModule);

            long totalEnergyOutput = this.reactors.Values.Sum(r => r.TotalEnergyOutput);

            long totalHeatAbsorbing = this.reactors.Values.Sum(r => r.TotalHeatAbsorbing);
